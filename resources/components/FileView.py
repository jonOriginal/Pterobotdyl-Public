...
class FileView(discord.ui.View):
    ...
    async def download(self, button, interaction):
        link = self.bot[str(interaction.guild_id)
                        ].get_file_download(self.filepath)

        embed = discord.Embed(title="Download Link",
                              description=f"[{self.custom_id}]({link})")
    ...
    async def view(self, button, interaction):
        ...
        data = self.bot[str(interaction.guild.id)
                        ].file_contents(f'{self.filepath}')
        ...
        paragraphs = [
            f'```{string[i:i + 1990]}```' for i in range(0, len(string), 1990)]
    ...
    async def delete(self, button, interaction):
        if self.confirmation == 0:
            self.confirmation = 1
            embed = discord.Embed(
                title=f'{self.custom_id[:79]},{self.size/1000}kb', description='Push again to confirm')

        elif self.confirmation == 1:
            self.bot[str(interaction.guild_id)].delete_file(self.filepath)
            embed = discord.Embed(
                title=f'{self.custom_id[:79]},{self.size / 1000}kb', description='File deleted', color=discord.Color.red())
